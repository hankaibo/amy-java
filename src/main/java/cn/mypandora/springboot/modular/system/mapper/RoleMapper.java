...
public interface RoleMapper extends MyBaseMapper<Role> {
     ...
     * @return 用户所有角色
     */
    List<Role> listByUserIdOrName(@Param("userId") Long userId, @Param("username") String username, @Param("status") Integer status);
     ...
     *
     * @param id     角色id
     * @param amount 大于id左值的角色，加上的数值(正数相当于加，负数相当于减)
     */
     ...
     *
     * @param id     角色id
     * @param amount 大于id右值的角色，加上的数值(正数相当于加，负数相当于减)
     * @param range  范围值
     */
